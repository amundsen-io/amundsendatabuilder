...
bigquery = ["google-api-python-client>=1.6.0, <2.0.0dev", "google-auth-httplib2>=0.0.1" "google-auth>=1.0.0, <2.0.0dev"]

jsonpath = ["jsonpath_rw==1.4.0"]

db2 = ["ibm_db==3.0.1", "ibm-db-sa-py3==0.3.1-1"]
...
all_deps = (
    requirements + kafka + cassandra + glue + snowflake + athena + bigquery + jsonpath + db2 + dremio + druid + spark
)
