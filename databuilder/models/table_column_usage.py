...
class ColumnReader(object):
    ...
    def __init__(
        self, database: str, cluster: str, schema: str, table: str, column: str, user_email: str, read_count: int = 1
    ) -> None:
...
ColumnReader(database={!r}, cluster={!r}, schema={!r}, table={!r}, column={!r}, user_email={!r}, read_count={!r})""".format(
            self.database, self.cluster, self.schema, self.table, self.column, self.user_email, self.read_count
        )
    ...
    def _create_rel_iterator(self) -> Iterator[GraphRelationship]:
        for col_reader in self.col_readers:
            relationship = GraphRelationship(
                ...
                attributes={TableColumnUsage.READ_RELATION_COUNT: col_reader.read_count},
            )
            yield relationship

    def _get_table_key(self, col_reader: ColumnReader) -> str:
        return TableMetadata.TABLE_KEY_FORMAT.format(
            db=col_reader.database, cluster=col_reader.cluster, schema=col_reader.schema, tbl=col_reader.table
        )
