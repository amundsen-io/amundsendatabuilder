...
class Neo4jStalenessRemovalTask(Task):
    ...
    def init(self, conf: ConfigTree) -> None:
        conf = Scoped.get_scoped_conf(conf, self.get_scope()).with_fallback(conf).with_fallback(DEFAULT_CONFIG)
    ...
    def _decorate_staleness(self, statement: str) -> str:
        """
        ...
        if self.ms_to_expire:
            ...
            OR NOT EXISTS(n.publisher_last_updated_epoch_ms)""".format(
                        marker=MARKER_VAR_NAME
                    )
                )
            )
        ...
        OR NOT EXISTS(n.published_tag)""".format(
                    marker=MARKER_VAR_NAME
                )
            )
        )
    ...
    def _batch_delete(self, statement: str, targets: Iterable[str]) -> None:
        """
        ...
        for t in targets:
            ...
            total_count = 0
            while True:
                results = self._execute_cypher_query(
                    statement=statement.format(type=t),
                    param_dict={"batch_size": self.batch_size, MARKER_VAR_NAME: self.marker},
    ...
    def _validate_staleness_pct(
        self, total_records: Iterable[Dict[str, Any]], stale_records: Iterable[Dict[str, Any]], types: Iterable[str]
    ) -> None:
        ...
        for record in stale_records:
            ...
            if stale_pct >= threshold:
                raise Exception(
                    "Staleness percentage of {} is {} %. Stopping due to over threshold {} %".format(
                        type_str, stale_pct, threshold
                    )
                )
    ...
    def _validate_node_staleness_pct(self) -> None:
        ...
        stale_records = self._execute_cypher_query(
            statement=stale_nodes_statement, param_dict={MARKER_VAR_NAME: self.marker}
        )
        self._validate_staleness_pct(total_records=total_records, stale_records=stale_records, types=self.target_nodes)
    ...
    def _validate_relation_staleness_pct(self) -> None:
        ...
        stale_records = self._execute_cypher_query(
            statement=stale_relations_statement, param_dict={MARKER_VAR_NAME: self.marker}
        )
        self._validate_staleness_pct(
            total_records=total_records, stale_records=stale_records, types=self.target_relations
        )

    def _execute_cypher_query(
        self, statement: str, param_dict: Dict[str, Any] = {}, dry_run: bool = False
    ) -> Iterable[Dict[str, Any]]:
        LOGGER.info(
            "Executing Cypher query: {statement} with params {params}: ".format(statement=statement, params=param_dict)
        )
