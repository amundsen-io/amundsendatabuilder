...
class FsNeo4jCSVLoader(Loader):
    ...
    _DEFAULT_CONFIG = ConfigFactory.from_dict({SHOULD_DELETE_CREATED_DIR: True, FORCE_CREATE_DIR: False})
    ...
    def init(self, conf: ConfigTree) -> None:
        ...
        self._relation_dir = conf.get_string(FsNeo4jCSVLoader.RELATION_DIR_PATH)

        self._delete_created_dir = conf.get_bool(FsNeo4jCSVLoader.SHOULD_DELETE_CREATED_DIR)
    ...
    def load(self, csv_serializable: GraphSerializable) -> None:
        ...
        while node:
            ...
            node_writer = self._get_writer(node_dict, self._node_file_mapping, key, self._node_dir, file_suffix)
        ...
        while relation:
            ...
            key2 = (relation.start_label, relation.end_label, relation.type, len(relation_dict))

            file_suffix = "{}_{}_{}".format(key2[0], key2[1], key2[2])
            relation_writer = self._get_writer(
                relation_dict, self._relation_file_mapping, key2, self._relation_dir, file_suffix
            )
