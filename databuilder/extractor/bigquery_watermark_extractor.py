...
class BigQueryWatermarkExtractor(BaseBigQueryExtractor):
    ...
    def _get_partitions(self, table: str, tableRef: Dict[str, str]) -> List[PartitionInfo]:
        ...
        body = {
            "query": query.format(
                project=tableRef["projectId"], dataset=tableRef["datasetId"], table=tableRef["tableId"]
            ),
            "useLegacySql": True,
        }
    ...
    def _get_partition_watermarks(
        self, table: Dict[str, Any], tableRef: Dict[str, str], partitions: List[PartitionInfo]
