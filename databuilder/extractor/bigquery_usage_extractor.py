...
TableColumnUsageTuple = namedtuple(
    "TableColumnUsageTuple", ["database", "cluster", "schema", "table", "column", "email"]
)
...
class BigQueryTableUsageExtractor(BaseBigQueryExtractor):
    ...
    def init(self, conf: ConfigTree) -> None:
        ...
        self.timestamp = conf.get_string(
            BigQueryTableUsageExtractor.TIMESTAMP_KEY, (date.today() - timedelta(days=1)).strftime("%Y-%m-%dT00:00:00Z")
        )
        ...
        for entry in self._retrieve_records():
            ...
            if refTables:
                if "totalTablesProcessed" in job["jobStatistics"]:
                    self._create_records(
                        refTables, job["jobStatistics"]["totalTablesProcessed"], email, job["jobName"]["jobId"]
                    )
            ...
            if refViews:
                if "totalViewsProcessed" in job["jobStatistics"]:
                    self._create_records(
                        refViews, job["jobStatistics"]["totalViewsProcessed"], email, job["jobName"]["jobId"]
                    )

    def _create_records(self, refResources: List[dict], resourcesProcessed: int, email: str, jobId: str) -> None:
    ...
    def _retrieve_records(self) -> Iterator[Optional[Dict]]:
        ...
        """
        body = {
            "resourceNames": ["projects/{project_id}".format(project_id=self.project_id)],
    ...
    def _page_over_results(self, body: Dict) -> Iterator[Dict]:
        response = (
            self.logging_service.entries().list(body=body).execute(num_retries=BigQueryTableUsageExtractor.NUM_RETRIES)
        )
        while response:
            ...
            try:
                if "nextPageToken" in response:
                    ...
                    response = (
                        self.logging_service.entries()
                        .list(body=body)
                        .execute(num_retries=BigQueryTableUsageExtractor.NUM_RETRIES)
                    )
                else:
                    response = None
            except Exception:
