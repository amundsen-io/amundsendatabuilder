...
class ModeDashboardUserExtractor(Extractor):
    ...
    def init(self, conf: ConfigTree) -> None:
        ...
        remove_fields_transformer.init(
            conf=Scoped.get_scoped_conf(self._conf, remove_fields_transformer.get_scope()).with_fallback(
                ConfigFactory.from_dict({FIELD_NAMES: ["organization", "mode_user_resource_path", "product"]})
            )
        )
    ...
    def _build_restapi_query(self) -> RestApiQuery:
        ...
        params = {
            "auth": HTTPBasicAuth(self._conf.get_string(MODE_ACCESS_TOKEN), self._conf.get_string(MODE_PASSWORD_TOKEN))
        }
        ...
        mode_user_ids_query = RestApiQuery(
