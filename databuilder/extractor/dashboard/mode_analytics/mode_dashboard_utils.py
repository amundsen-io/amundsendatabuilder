...
class ModeDashboardUtils(object):
    ...
    def get_spaces_query_api(conf: ConfigTree) -> BaseRestApiQuery:
        ...
        # Spaces
        params = {"auth": HTTPBasicAuth(conf.get_string(MODE_ACCESS_TOKEN), conf.get_string(MODE_PASSWORD_TOKEN))}
    ...
    @staticmethod
    def create_mode_rest_api_extractor(restapi_query: BaseRestApiQuery, conf: ConfigTree) -> RestAPIExtractor:
        """
        ...
        rest_api_extractor_conf = (
            ...
            .with_fallback(conf)
            .with_fallback(
                ConfigFactory.from_dict({REST_API_QUERY: restapi_query, STATIC_RECORD_DICT: {"product": "mode"}})
            )
        )
