...
class BaseBigQueryExtractor(Extractor):
    ...
    def init(self, conf: ConfigTree) -> None:
        ...
        self.pagesize = conf.get_int(BaseBigQueryExtractor.PAGE_SIZE_KEY, BaseBigQueryExtractor.DEFAULT_PAGE_SIZE)
    ...
    def _page_dataset_list_results(self) -> Iterator[Any]:
        ...
        while response:
            ...
            if "nextPageToken" in response:
                response = (
                    self.bigquery_service.datasets()
                    .list(projectId=self.project_id, all=True, filter=self.filter, pageToken=response["nextPageToken"])
                    .execute(num_retries=BaseBigQueryExtractor.NUM_RETRIES)
                )
            else:
                response = None
    ...
    def _page_table_list_results(self, dataset: DatasetRef) -> Iterator[Dict[str, Any]]:
        response = (
            ...
            .list(projectId=dataset.projectId, datasetId=dataset.datasetId, maxResults=self.pagesize)
            .execute(num_retries=BaseBigQueryExtractor.NUM_RETRIES)
        )
