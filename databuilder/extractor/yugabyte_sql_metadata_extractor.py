...
class YugabyteSqlMetadataExtractor(Extractor):
    """
    Extracts yugabyte SQL table and column metadata from underlying meta store database using SQLAlchemyExtractor
    """
    # SELECT statement from yugabyte information_schema to extract table and column metadata
    SQL_STATEMENT = """
    SELECT
    ...
    def init(self, conf):
        ...
        conf = conf.with_fallback(YugabyteSqlMetadataExtractor.DEFAULT_CONFIG)
        self._cluster = '{}'.format(conf.get_string(YugabyteSqlMetadataExtractor.CLUSTER_KEY))

        if conf.get_bool(YugabyteSqlMetadataExtractor.USE_CATALOG_AS_CLUSTER_NAME):
            cluster_source = "c.table_catalog"
        else:
            cluster_source = "'{}'".format(self._cluster)

        self._database = conf.get_string(YugabyteSqlMetadataExtractor.DATABASE_KEY, default='yugabyte')

        self.sql_stmt = YugabyteSqlMetadataExtractor.SQL_STATEMENT.format(
            where_clause_suffix=conf.get_string(YugabyteSqlMetadataExtractor.WHERE_CLAUSE_SUFFIX_KEY),
        ...
        LOGGER.info('SQL for yugabyte metadata: {}'.format(self.sql_stmt))
    ...
    def get_scope(self):
        # type: () -> str
        return 'extractor.yugabyte_sql_metadata'
