...
class TestRestApiQuery(unittest.TestCase):
    def test_rest_api_query_seed(self) -> None:
        rest_api_query = RestApiQuerySeed(seed_record=[{"foo": "bar"}, {"john": "doe"}])
    ...
    def test_rest_api_query(self) -> None:
        ...
        with patch("databuilder.rest_api.rest_api_query.requests.get") as mock_get:
            ...
            query = RestApiQuery(
                query_to_join=seed_query, url="foobar", params={}, json_path=json_path, field_names=field_names
            )

            expected = [{"name_field": "john", "foo1": "bar1"}, {"name_field": "doe", "foo2": "bar2"}]
    ...
    def test_compute_subresult_single_field(self) -> None:
        ...
        sub_records = RestApiQuery._compute_sub_records(
            result_list=["1", "2", "3"], field_names=["foo"], json_path_contains_or=True
        )
    ...
    def test_compute_subresult_multiple_fields_json_path_or_expression(self) -> None:
        sub_records = RestApiQuery._compute_sub_records(
            result_list=["1", "2", "3", "a", "b", "c"], field_names=["foo", "bar"], json_path_contains_or=True
        )
