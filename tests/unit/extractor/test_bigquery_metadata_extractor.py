        ...
        "fields": [
            {
                "name": "nested",
                "type": "RECORD",
                "fields": [{"name": "nested2", "type": "RECORD", "fields": [{"name": "ahah", "type": "STRING"}]}],
            }
        ]
    },
    "type": "TABLE",
...
class MockBigQueryClient:
    def __init__(self, dataset_list_data: Any, table_list_data: Any, table_data: Any) -> None:
...
class TestBigQueryMetadataExtractor(unittest.TestCase):
    def setUp(self) -> None:
        config_dict = {
            "extractor.bigquery_table_metadata.{}".format(BigQueryMetadataExtractor.PROJECT_ID_KEY): "your-project-here"
        }
    ...
    def test_can_handle_datasets(self, mock_build: Any) -> None:
        ...
        extractor.init(Scoped.get_scoped_conf(conf=self.conf, scope=extractor.get_scope()))
    ...
    def test_accepts_dataset_filter_by_label(self, mock_build: Any) -> None:
        config_dict = {
            ...
            "extractor.bigquery_table_metadata.{}".format(BigQueryMetadataExtractor.FILTER_KEY): "label.key:value",
        }
    ...
    def test_keypath_and_pagesize_can_be_set(self, mock_build: Any) -> None:
        config_dict = {
            ...
            "extractor.bigquery_table_metadata.{}".format(BigQueryMetadataExtractor.PAGE_SIZE_KEY): 200,
            "extractor.bigquery_table_metadata.{}".format(BigQueryMetadataExtractor.KEY_PATH_KEY): "/tmp/doesnotexist",
        }
        conf = ConfigFactory.from_dict(config_dict)
