...
from databuilder.extractor.delta_lake_metadata_extractor import (
    DeltaLakeMetadataExtractor,
    ScrapedTableMetadata,
...
class TestDeltaLakeExtractor(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.spark = (
            SparkSession.builder.appName("Amundsen Delta Lake Metadata Extraction")
            .master("local")
    ...
    def setUpSchemas(self) -> None:
        ...
        self.spark.sql(
            "create table if not exists " "test_schema1.test_table3 (c boolean, d float) using delta partitioned by (c)"
        )
    ...
    def test_fetch_delta_columns(self) -> None:
        ...
        expected = [
            ScrapedColumnMetadata(name="a", description=None, data_type="string", sort_order=0),
    ...
    def test_scrape_tables(self) -> None:
        table = Table(
            name="test_table1", database="test_schema1", description=None, tableType="delta", isTemporary=False
        )
    ...
    def test_create_table_metadata(self) -> None:
        ...
        expected = TableMetadata(
            ...
            "test_table1",
            description=None,
            columns=[ColumnMetadata("a", None, "string", 0), ColumnMetadata("b", None, "int", 1)],
        )
    ...
    def test_table_does_not_exist(self) -> None:
        table = Table(
            name="test_table5", database="test_schema1", description=None, tableType="delta", isTemporary=False
        )
        actual = self.dExtractor.scrape_table(table)
    ...
    def test_scrape_all_tables(self) -> None:
        tables = [
            Table(name="test_table1", database="test_schema1", description=None, tableType="delta", isTemporary=False),
            Table(name="test_table3", database="test_schema1", description=None, tableType="delta", isTemporary=False),
        ]
