...
class TestRestAPIExtractor(unittest.TestCase):
    def test_static_data(self) -> None:
        ...
        conf = ConfigFactory.from_dict(
            {REST_API_QUERY: RestApiQuerySeed(seed_record=[{"foo": "bar"}]), STATIC_RECORD_DICT: {"john": "doe"}}
        )
    ...
    def test_model_construction(self) -> None:
        ...
        record = extractor.extract()
        expected = DashboardMetadata(
            dashboard_group="foo", dashboard_name="bar", description="john", dashboard_group_description="doe"
        )
