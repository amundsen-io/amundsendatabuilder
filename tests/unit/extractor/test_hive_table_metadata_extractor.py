...
class TestHiveTableMetadataExtractor(unittest.TestCase):
    def setUp(self) -> None:
        ...
        config_dict = {"extractor.sqlalchemy.{}".format(SQLAlchemyExtractor.CONN_STRING): "TEST_CONNECTION"}
    ...
    def test_extraction_with_empty_query_result(self) -> None:
        ...
        """
        with patch.object(SQLAlchemyExtractor, "_get_connection"), patch.object(
    ...
    def test_extraction_with_single_result(self) -> None:
        with patch.object(SQLAlchemyExtractor, "_get_connection") as mock_connection, patch.object(
            HiveTableMetadataExtractor,
            "_choose_default_sql_stm",
            return_value=HiveTableMetadataExtractor.DEFAULT_SQL_STATEMENT,
        ):
            ...
            table = {"schema": "test_schema", "name": "test_table", "description": "a table for testing", "is_view": 0}
    ...
    def test_extraction_with_multiple_result(self) -> None:
        with patch.object(SQLAlchemyExtractor, "_get_connection") as mock_connection, patch.object(
            ...
            sql_execute = MagicMock()
            connection.execute = sql_execute
            table = {"schema": "test_schema1", "name": "test_table1", "description": "test table 1", "is_view": 0}

            table1 = {"schema": "test_schema1", "name": "test_table2", "description": "test table 2", "is_view": 0}

            table2 = {"schema": "test_schema2", "name": "test_table3", "description": "test table 3", "is_view": 0}
            ...
            expected = TableMetadata(
                ...
                "test table 1",
                [
                    ColumnMetadata("col_id1", "description of col_id1", "bigint", 0, ["partition column"]),
            ...
            expected = TableMetadata(
                ...
                [
                    ...
                    ColumnMetadata("col_name3", "description of col_name3", "varchar", 1),
                ],
                is_view=False,
            )
            self.assertEqual(expected.__repr__(), extractor.extract().__repr__())
    ...
    def test_hive_sql_statement_with_custom_sql(self) -> None:
        ...
        with patch.object(SQLAlchemyExtractor, "_get_connection"), patch.object(
            ...
            config_dict = {
                HiveTableMetadataExtractor.WHERE_CLAUSE_SUFFIX_KEY: self.where_clause_suffix,
                "extractor.sqlalchemy.{}".format(SQLAlchemyExtractor.CONN_STRING): "TEST_CONNECTION",
                HiveTableMetadataExtractor.EXTRACT_SQL: "select sth for test {where_clause_suffix}",
            }
