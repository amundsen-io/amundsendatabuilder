...
class TestTableSource(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.table_source = TableSource(db_name="hive", schema=SCHEMA, table_name=TABLE, cluster=CLUSTER, source=SOURCE)

    def test_get_source_model_key(self) -> None:
        source = self.table_source.get_source_model_key()
        self.assertEqual(
            source,
            "{db}://{cluster}.{schema}/{tbl}/_source".format(
                db=DB,
                schema=SCHEMA,
                tbl=TABLE,
                cluster=CLUSTER,
    ...
    def test_create_relation(self) -> None:
        ...
        start_key = "{db}://{cluster}.{schema}/{tbl}/_source".format(db=DB, schema=SCHEMA, tbl=TABLE, cluster=CLUSTER)
        end_key = "{db}://{cluster}.{schema}/{tbl}".format(db=DB, schema=SCHEMA, tbl=TABLE, cluster=CLUSTER)
