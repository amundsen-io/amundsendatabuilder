...
class TestTableLineage(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.table_lineage = TableLineage(
            ...
            table_name=TABLE,
            cluster=CLUSTER,
            downstream_deps=["hive://default.test_schema/test_table1", "hive://default.test_schema/test_table2"],
        )

    def test_get_table_model_key(self) -> None:
        metadata = self.table_lineage.get_table_model_key(db=DB, cluster=CLUSTER, schema=SCHEMA, table=TABLE)
    ...
    def test_create_relation(self) -> None:
        ...
        start_key = "{db}://{cluster}.{schema}/{tbl}".format(db=DB, schema=SCHEMA, tbl=TABLE, cluster=CLUSTER)
        ...
        actual_relations = [neo4_serializer.serialize_relationship(relation) for relation in relations]
