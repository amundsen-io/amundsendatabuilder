...
with DAG("amundsen_databuilder", default_args=default_args, **dag_args) as dag:
    ...
    snowflake_table_metadata_job = PythonOperator(
        task_id="snowflake_table_metadata_extract_job", python_callable=create_snowflake_table_metadata_job
    )

    snowflake_es_publisher_job = PythonOperator(
        task_id="snowflake_es_publisher_job", python_callable=create_snowflake_es_publisher_job
    )
