...
def create_table_extract_job():
    ...
    job_config = ConfigFactory.from_dict(
        {
            "extractor.athena_metadata.{}".format(AthenaMetadataExtractor.WHERE_CLAUSE_SUFFIX_KEY): where_clause_suffix,
            "extractor.athena_metadata.extractor.sqlalchemy.{}".format(
...
with DAG("amundsen_databuilder", default_args=default_args, **dag_args) as dag:
    athena_table_extract_job = PythonOperator(
        task_id="athena_table_extract_job", python_callable=create_table_extract_job
    )

    athena_es_publisher_job = PythonOperator(
        task_id="athena_es_publisher_job", python_callable=create_es_publisher_sample_job
    )
