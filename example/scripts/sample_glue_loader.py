...
def create_glue_extractor_job():
    ...
    job_config = ConfigFactory.from_dict(
        {
            "extractor.glue.{}".format(GlueExtractor.CLUSTER_KEY): GLUE_CLUSTER_KEY,
            ...
            "publisher.neo4j.{}".format(neo4j_csv_publisher.NEO4J_USER): NEO4j_USERNAME,
            "publisher.neo4j.{}".format(neo4j_csv_publisher.NEO4J_PASSWORD): NEO4j_PASSWORD,
            "publisher.neo4j.{}".format(neo4j_csv_publisher.JOB_PUBLISH_TAG): str(int(datetime.utcnow().timestamp())),
        }
    )

    return DefaultJob(
        conf=job_config,
        task=DefaultTask(extractor=GlueExtractor(), loader=FsNeo4jCSVLoader(), transformer=NoopTransformer()),
...
def create_es_publisher_job():
    ...
    job_config = ConfigFactory.from_dict(
        {
            ...
            "extractor.search_data.extractor.neo4j.{}".format(Neo4jExtractor.NEO4J_AUTH_USER): NEO4j_USERNAME,
    ...
    return DefaultJob(conf=job_config, task=task, publisher=ElasticsearchPublisher())
