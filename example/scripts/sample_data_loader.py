...
def run_csv_job(file_loc, job_name, model):
    ...
    task = DefaultTask(extractor=csv_extractor, loader=csv_loader, transformer=NoopTransformer())
    ...
    DefaultJob(conf=job_config, task=task, publisher=Neo4jCsvPublisher()).launch()
...
def run_table_column_job(table_path, column_path):
    ...
    csv_loader = FsNeo4jCSVLoader()
    task = DefaultTask(extractor, loader=csv_loader, transformer=NoopTransformer())
    job_config = ConfigFactory.from_dict(
        {
...
def create_last_updated_job():
    ...
    return DefaultJob(conf=job_config, task=task, publisher=Neo4jCsvPublisher())
...
def create_dashboard_tables_job():
    ...
    transformer = ChainedTransformer(
        transformers=[generic_transformer, dict_to_model_transformer], is_init_transformers=True
    )

    task = DefaultTask(extractor=csv_extractor, loader=csv_loader, transformer=transformer)
    ...
    job_config = ConfigFactory.from_dict(
        {
            ...
            "{}.{}.{}".format(
                transformer.get_scope(), generic_transformer.get_scope(), CALLBACK_FUNCTION
            ): _str_to_list,
            "{}.{}.{}".format(
                transformer.get_scope(), dict_to_model_transformer.get_scope(), MODEL_CLASS
...
def create_es_publisher_sample_job(
    ...
    task = DefaultTask(
        loader=FSElasticsearchJSONLoader(), extractor=Neo4jSearchDataExtractor(), transformer=NoopTransformer()
    )
...
if __name__ == "__main__":
    ...
    if create_connection(DB_FILE):
        ...
        run_csv_job("example/sample_data/sample_user.csv", "test_user_metadata", "databuilder.models.user.User")
        run_csv_job(
        ...
        run_csv_job(
            "example/sample_data/sample_tags.csv", "test_tag_metadata", "databuilder.models.table_metadata.TagMetadata"
        )
        run_csv_job(
